...
def test_token_signed_with_a_different_algorithm_is_rejected():
    ...
    library has since been replaced with PyJWT, but the defence that matters is
    the explicit algorithms= allowlist passed to jwt.decode — pin it so a future
...
# ---------------------------------------------------------------------------
# Password hashing — passlib -> bcrypt migration
# ---------------------------------------------------------------------------

# Generated by the passlib+bcrypt stack this project shipped with. Every stored
# password_hash looks like this, so it must keep verifying after the swap or
# every existing user is locked out.
PASSLIB_ERA_HASH = "$2b$12$La435C147PN2JbXWhkdaCOrQrUBfP9x04E2OgldFc2ZnvmPQ5lznK"


def test_hashes_written_by_passlib_still_verify():
    assert verify_password("password123", PASSLIB_ERA_HASH) is True


def test_hashes_written_by_passlib_reject_wrong_password():
    assert verify_password("not-the-password", PASSLIB_ERA_HASH) is False


def test_new_hash_uses_the_same_bcrypt_format():
    """Still $2b$, so a rollback to the old code can read new hashes too."""
    assert get_password_hash("whatever").startswith("$2b$")


def test_password_longer_than_bcrypt_limit_is_accepted():
    """bcrypt only considers the first 72 bytes and 4.x raises on longer input.

    passlib truncated silently, so anyone who set a longer password is stored
    against its first 72 bytes. Raising here would lock them out.
    """
    long_password = "x" * 100
...
def test_password_differing_only_past_the_bcrypt_limit_still_matches():
    """Documents the consequence of that truncation rather than hiding it."""
    hashed = get_password_hash("y" * 72)
    assert verify_password("y" * 72 + "extra", hashed) is True
