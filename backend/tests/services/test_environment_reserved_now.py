...
async def _booking(db, tenant_id, env_id, status: str, *, covers_now: bool = True, covers_past: bool = False):
...
@pytest.mark.asyncio
async def test_an_already_finished_booking_does_not_reserve(db_session, test_tenant):
    """A booking whose start and end are both in the past must not reserve."""
    env = await _environment(db_session, test_tenant.id, "env-past")
    await _booking(db_session, test_tenant.id, env.id, "approved", covers_past=True)
