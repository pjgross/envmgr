...
**Architecture:** The 1024 px width check found the app clean everywhere MUI's `DataGrid` renders: it owns a virtual scroller and its column headers translate with it, so all 30 migrated grids scroll internally at 784 px of content width. The defect is in the **21 files holding 26 raw `<Table>` instances** the PR 4 migration never touched, because a bare MUI `<Table>` has no scroll container of its own — it simply grows and pushes the document. Five files (9 instances) already wrap every `<Table>` in `TableContainer` (which sets `overflowX: 'auto'`); **16 files (17 instances) do not**, and two were proved to overflow with ordinary data. The fix is one line at each instance — wrap the `<Table>` in a bare `<TableContainer>` — plus a source sweep that keeps the count of `<Table>` and `<TableContainer>` equal in every file, the same shape as the `storageKey` sweep PR 4 introduced.

**Tech Stack:** React 18, TypeScript (strict), MUI v5, Redux Toolkit, Vitest + Testing Library, ESLint 8 (`.eslintrc.cjs`, eslintrc format — **not** flat config).
