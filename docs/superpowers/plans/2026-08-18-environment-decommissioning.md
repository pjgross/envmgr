...
- [ ] **Step 2: `CLAUDE.md`** — a B5 block in the same voice as B4's, plus these new **Common Pitfalls** entries:
  - *Building a thing and connecting it to nothing.* **B5 shipped this FOUR times**, each caught by a different accident and none by a test: `idle` computed and filterable but absent from the response schema; `decommission_state` heading the same way until the plan was amended; `initiateDecommission` defined in the slice with **no caller anywhere**, so the workflow could not be started from the product at all; and `environment_tier.idle_threshold_days` — a model column with no schema field and no service handling, leaving the per-tier override unsettable while `COALESCE(tier, tenant)` silently always chose the tenant default. Every piece worked; nothing joined them. **The question that finds these is "what consumes this?", asked of every new field, thunk and column — not "does this pass?"**
