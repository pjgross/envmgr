...
**2. Incidents in the window.** A count per severity (P1–P4) and a list of the incidents whose **cause** is this release and whose detection time falls inside the window — from the phase's start to the earliest of *declared stable*, the phase's end, and now. Each row links to the incident. This card is shown once the window has opened — that is, once the hyper-care phase's start date has arrived — and stays hidden while there is no phase at all, or while one is only *Planned*, because in both cases there is no window yet to report on.
