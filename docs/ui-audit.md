...
| P2-2 | **Silent list-fetch failures** — `BuildList`, `DeploymentList`, `ReleaseList` never read their slice's `error`, so a failed fetch is an unexplained empty grid. | `BuildList.tsx`, `DeploymentList.tsx`, `ReleaseList.tsx` (errors exist in the slices) | Render an `<Alert severity="error">` from the slice error. | ✅ Closed (PR 4). All three render their slice `error` as an `<Alert severity="error">`. That alone wasn't enough: each grid's entity-specific empty message ("No builds match these filters.") still stated a fact the app doesn't know on a failed fetch, so `emptyMessage` is now conditional on `error` too — not anticipated by the original finding. The three list thunks also now `rejectWithValue(formatApiError(err))`, because RTK's default error serializer drops `response.data.detail`; without it the alert would have read "Request failed with status code 500" instead of the server's reason. `BookingList.tsx` has the same unconditional-`emptyMessage` shape and is **not** covered — it falls outside this finding's three named pages. |
