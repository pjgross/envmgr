...
|---|---------|-------|-----|--------|
| A1 | **Icon-only buttons with no accessible name.** Some have neither label nor tooltip; many rely on a `<Tooltip>`, which sets `aria-describedby` on hover — **not** an accessible *name*. Screen readers announce "button". Runtime pass confirmed ≥1 unnamed button on the release detail; there is no `<h1>` on any page either. | No-name: `AppLayout.tsx:177`, `EnvSubsystemHostsDialog.tsx:196`, `ScopeTable.tsx:205`. Tooltip-only (needs `aria-label` too): `EnvironmentList.tsx:208,213`, `SystemCatalog.tsx:177,182`, `ScopeTable.tsx:181,195`, `ApiKeyManagement.tsx:80`, many in `SystemDetail.tsx` | Add an explicit `aria-label` to every icon button (in addition to the Tooltip). Set `component="h1"` on the primary page title. | 🟡 Partially closed (PR 4) — **and this finding's own mechanism is wrong.** Checked twice against `frontend/node_modules/@mui/material/Tooltip/Tooltip.js`: `Tooltip` defaults `describeChild` to `false`, and in that branch (lines 453-460) a string `title` sets `aria-label` on the child, not `aria-describedby` — `aria-describedby` only appears when `describeChild` is opted into, which nothing here does. So most of the "Tooltip-only" sites listed already had a name; the real gap is narrower and falls into three shapes: an icon button with **no** Tooltip at all; a Tooltip whose text is **conditional on row data**, so the accessible name drifts with the data; and — found doing this work — `<Tooltip><span><IconButton>` (MUI's disabled-button wrapper), where the tooltip's fallback name lands on the `<span>` and can **never** reach the inner `<button>`, making an explicit label the only possible source of a name (`ReleaseEventTypesPanel`'s Delete button is exactly this shape). A tooltip-wrapped button whose `aria-label` merely repeats its tooltip words can't be guarded by a test either way — remove the label and MUI's own fallback supplies the same name — so only these three categories are covered. Every icon-only button in the 28 grid files migrated by this PR now carries an explicit `aria-label` matching its tooltip's words, plus a 36px hit area (`sx={{ p: 1 }}` is 8px of padding on each side against the theme's default 8px spacing unit, and `<EditIcon fontSize="small" />` renders at 20px — `.MuiSvgIcon-fontSizeSmall` is `pxToRem(20)`, which beats the `size="small"` IconButton's own inherited 18px — so 8 + 20 + 8 = 36px, comfortably clear of WCAG 2.2's 24px minimum). Still open: `SystemDetail`, `EnvSubsystemHostsDialog`, `ScopeTable`, `AppLayout` and the doc's other non-grid surfaces. |
...
| P2-2 | **Silent list-fetch failures** — `BuildList`, `DeploymentList`, `ReleaseList` never read their slice's `error`, so a failed fetch is an unexplained empty grid. | `BuildList.tsx`, `DeploymentList.tsx`, `ReleaseList.tsx` (errors exist in the slices) | Render an `<Alert severity="error">` from the slice error. | ✅ Closed (PR 4). All three render their slice `error` as an `<Alert severity="error">`. That alone wasn't enough: each grid's entity-specific empty message ("No builds match these filters.") still stated a fact the app doesn't know on a failed fetch, so `emptyMessage` is now conditional on `error` too — not anticipated by the original finding. The three list thunks also now `rejectWithValue(formatApiError(err))`, because RTK's default error serializer drops `response.data.detail`; without it the alert would have read "Request failed with status code 500" instead of the server's reason. Eight pages have the same unconditional-`emptyMessage` shape and are **not** covered — they fall outside this finding's three named pages: `BookingList.tsx`, `EnvironmentList.tsx`, `SystemCatalog.tsx`, `InfrastructureComponentList.tsx`, `EnvironmentRequestList.tsx`, `DecommissionWorklist.tsx`, `EscalationWorklist.tsx`, `PirActionList.tsx`. |
...
| P2-5 | **Raw `<DataGrid>` in 15 files vs the `DataTable` wrapper in 4.** The wrapper adds saved column visibility, consistent density, and empty-state text — most tables miss all of it. | `BookingList`, `EnvironmentList`, `BuildList`, `SystemCatalog`, `DeploymentList`, `ApiKeyManagement`, +admin/enterprise | Migrate raw-DataGrid list pages to `components/DataTable.tsx`. | ✅ Closed (PR 4). The real count was 28 files (30 grids), not the 15 this finding counted — all of them now render through `DataTable`. An ESLint `no-restricted-imports` rule forbids importing the `DataGrid` binding outside the wrapper (types stay unrestricted), and a source sweep test asserts every `storageKey` is unique. Also not anticipated by the finding: none of the 28 had rendered a toolbar, so a column hidden through the header menu could never be restored — every migrated grid has one now. |
