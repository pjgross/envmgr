...
>
> **Next**: Phase 6 (Infrastructure Topology) — plans against PostgreSQL, no graph store. Known open items: the pagination sweep bounded 20 list endpoints ([docs/pagination.md](docs/pagination.md)); 3 remain unbounded **blocked on a query restructure, not merely pending** — `GET /{release_id}/raid` and the two `dependencies` endpoints all filter or merge in Python *after* the query, so a `LIMIT` would window the wrong set until that's fixed; 5 rollup/report endpoints are permanently unbounded by design (aggregations, not row lists) and a handful of tenant-config lists are bounded in practice — neither needs action. Newly found: `GET /releases/calendar` and `/releases/timeline` call `list_releases` with a hardcoded `limit=500` and discard the total, silently truncating past 500 releases. env-topology SP4 (group-by-system/host toggle); `docs/architecture copy.md` could use a real filename (content is current — it is the macmini host map).
>
