...
> **Repo / remote (2026-07-22)**: Canonical remote is now **GitHub `github.com/pjgross/envmgr`** (private, default `main`) via git remote `github`. The old local GitLab (`origin`, `localhost:8929`) is **legacy/dev-only** and its `main` is stale — push and open PRs against **GitHub** (`gh` CLI, account `pjgross`). For where `main` actually is, run `git log -1 github/main` — **this file deliberately records no current tip SHA**, because a line naming HEAD is written by the very commit that changes HEAD and is therefore stale before it lands. (SHAs recorded *historically* below — "this programme ended at X" — are a different thing and stay accurate.)
