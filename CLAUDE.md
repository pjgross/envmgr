...
> - **Rank is PATCH-only.** `ProjectCreate` has no `priority_rank`, so every project is born unranked; `ProjectUpdate` is `extra="forbid"` + `ge=1` and keys on `model_fields_set`, so an omitted key means "leave alone" and only an explicit null unranks.
...
> - **The group name is part of what identifies a booking.** A decision naming one member of a group booking is a decision about every member (A2 transitions atomically), so the note renders **per row**, only where that side has a group — a blanket page-level hedge fires on the majority of rows that have none.
...
> - **`GET /bookings/{id}/conflicts` now runs four A4 batches beside its existing three — and is still not fully batched**: `conflict_service.get_ack` remains one query per row. Pre-existing, not A4's, and disclosed in the code so the batching note there is not read as a claim about the endpoint.
