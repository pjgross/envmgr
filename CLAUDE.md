...
> - **The booking→project link is create-only** — `EditStandardFieldsDialog`'s fieldMap and `ENTITY_FIELD_SPECS["booking"]` both omit `project_id`, so a mislinked booking can only be fixed by recreating it. Harmless while A1 enforces nothing. **Fix it before A3**, or enforcement lands on a field no UI can correct.
