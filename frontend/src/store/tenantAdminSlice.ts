...
export const fetchTenantSettings = createAsyncThunk('tenantAdmin/fetchSettings', () => tenantAdminService.getSettings())
...
const tenantAdminSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchTenantSettings.fulfilled, (state, action) => { state.settings = action.payload; state.loading = false })
      .addCase(fetchTenantSettings.pending, (state) => { state.loading = true; state.error = null })
      .addCase(fetchTenantSettings.rejected, (state, action) => { state.loading = false; state.error = action.error.message ?? 'Failed' })
      .addCase(updateTenantSettings.fulfilled, (state, action) => { state.settings = action.payload; state.loading = false })
      .addCase(updateTenantSettings.pending, (state) => { state.loading = true; state.error = null })
      .addCase(updateTenantSettings.rejected, (state, action) => { state.loading = false; state.error = action.error.message ?? 'Failed' })
