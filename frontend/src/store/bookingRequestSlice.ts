...
const slice = createSlice({
  ...
  reducers: {},
  extraReducers: (b) => {
    b.addCase(fetchBookingRequests.pending, (s) => {
      s.loading = true;
      s.error = null;
    });
    ...
    b.addCase(
      createBookingRequest.fulfilled,
      (s, a: PayloadAction<BookingRequestCreateResponse>) => {
    ...
    b.addCase(
      updateRequestStandardFields.fulfilled,
      (s, a: PayloadAction<BookingRequestResponse>) => {
        const i = s.requests.findIndex((r) => r.id === a.payload.id);
        if (i !== -1) s.requests[i] = a.payload;
      }
    );
    b.addCase(
      updateRequestCustomFields.fulfilled,
      (s, a: PayloadAction<BookingRequestResponse>) => {
        const i = s.requests.findIndex((r) => r.id === a.payload.id);
        if (i !== -1) s.requests[i] = a.payload;
      }
