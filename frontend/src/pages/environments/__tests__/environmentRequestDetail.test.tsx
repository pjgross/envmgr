...
describe('EnvironmentRequestDetail', () => {
  ...
  it('renders the welcome pack once the request is fulfilled', async () => {
    ...
    expect(await screen.findByText('Welcome Pack')).toBeInTheDocument();
    ...
    // that hides the section instead of showing the fallback text.
    expect(screen.getAllByText('Not provided').length).toBeGreaterThan(0);
  });

  // A new-environment request cannot be fulfilled without an
  // operations_group_id (fulfilment 409s otherwise). The backend's
  // update_request only accepts a PATCH while status is 'draft' (see
  // environment_request_service.py's "can only be edited while it is a
  // draft" guard, and task-6-brief.md's fulfilment flow, which sets the
  // group BEFORE transitioning to submitted) — so the picker must gate on
  // draft, not submitted, or the PATCH would 409 every time it was used.
