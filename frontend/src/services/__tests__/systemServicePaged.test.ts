...
describe('systemService.listSystems', () => {
  ...
  it('falls back to the row count when the header is absent', async () => {
    ...
    expect((await systemService.listSystems({})).total).toBe(1);
  });

  it('forwards paging, sorting and filter params', async () => {
    // NOTE: `listSystems` is a pure passthrough — it wraps the params object
    ...
    // object literal passed to `listSystems`, which `npx tsc --noEmit`
    // enforces but `npx vitest run` (esbuild strips types, no type-check)
    // cannot. If a key here is ever renamed or dropped from `listSystems`'s
    // param type, this test will keep passing; only `tsc` will fail.
